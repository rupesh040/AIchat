        ...
        <div className='w-[100%] h-screen bg-zinc-300 flex  overflow-hidden ' onClick={() => setProfile(true)}>
            ...
            <div className="w-14 h-full max-[800px]:max-w-96 max-[800px]:hidden   bg-black border-r-2 border-zinc-900 flex flex-col pt-24 items-center justify-between pb-5 max-[800px]:flex-row max-[800px]:fixed max-[800px]:bg-zinc-900 max-[800px]:bottom-5 max-[800px]:right-0 max-[800px]:rounded-xl max-[800px]:left-0 max-[800px]:z-50 max-[800px]:h-14   max-[800px]:py-5 max-[800px]:px-3 max-[800px]:w-[90%] max-[800px]:mx-auto max-[800px]:border-[1px] max-[800px]:overflow-hidden">
                ...
                <div className="flex flex-col gap-3 max-[800px]:flex-row max-[800px]:justify-between max-[800px]:w-full px-1">
                    ...
                    <i className="ri-translate-ai-2 text-2xl  bg-zinc-300 cursor-pointer text-black  px-2 py-1 rounded-xl "></i>
            ...
            <div className='w-full  bg-black h-screen relative flex flex-col justify-center items-center '>
              {isResponse ? <> <h1 className='text-[50px] max-[490px]:text-[40px] font-black bg-gradient-to-r from-blue-500  via-purple-500 to-red-500 bg-clip-text text-transparent relative'>Hello Rupesh  </h1>
            ...
            <div className={`${profile ? "hidden" : "flex"} w-72  h-80 border-[1px] border-zinc-800 bg-[#18181b] overflow-hidden  backdrop-blur-lg shadow-2xl absolute bottom-16 min-[800px]:left-10 rounded-2xl p-5 bg-opacity-90  items-center flex-col max-[800px]:bottom-20 max-[800px]:right-0  z-30 mr-10 `} onClick={(e) => e.stopPropagation()}>
                <img src={setIMage || authUser.profilePic} className='w-32 h-32 bg-black  rounded-full border-4' alt="" />
                <h1 className='text-white font-black text-xl tracking-wide mt-2'>{authUser.fullname}</h1>
                <p className='text-white'>{authUser.email}</p>
                <div className="w-[90%] h-12  mt-5 flex flex-row gap-1">
                    {!setIMage ?
                        <label className={`w-[50%] h-full ${isUpdatingProfile ? "bg-[#c9571922] text-[#c95719]" : "bg-zinc-700 text-zinc-200"} rounded-l-2xl cursor-pointer flex justify-center items-center  gap-1`} > <i className="ri-edit-line text-zinc-200"></i>Edit <input type="file" hidden onChange={(e) => DpUpload(e)} /></label> :
                        <div className={`w-[50%] h-full text-lg ${isUpdatingProfile ? "bg-[#c9571922] text-[#c95719]" : "bg-zinc-700 text-zinc-200"} rounded-l-2xl cursor-pointer flex justify-center items-center  gap-1`} onClick={() => profileSave()}>{isUpdatingProfile ? <svg viewBox="25 25 50 50">
                            <circle r="20" cy="50" cx="50"></circle>
                        </svg> : null}  save</div>}
                    <div className={`w-[50%] h-full ${isLogout ? "bg-[#c9571922] text-[#c95719]" : "bg-zinc-700 text-zinc-200"} gap-1 rounded-r-2xl cursor-pointer flex justify-center items-center `} onClick={() => logout()}>
                        {isLogout ?
                            <svg viewBox="25 25 50 50">
                                <circle r="20" cy="50" cx="50"></circle>
                            </svg> : <i className="ri-logout-box-r-line text-zinc-200 "></i>} Logout  </div>
                </div>
